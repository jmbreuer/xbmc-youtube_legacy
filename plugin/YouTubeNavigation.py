...
class YouTubeNavigation():
	...
	def __init__(self):
		...
		self.categories = (
					  ...
					  {'Title':self.language( 30039 )  ,'path':"/root/explore/disco/popular"			, 'thumbnail':"discoball"		 	, 'login':"false" , 'scraper':"disco_top_artist", 'folder':'true'},
					  ...
					  {'Title':self.language( 30035 )  ,'path':"/root/explore/trailers/latest"   		, 'thumbnail':"trailers"			, 'login':"false" , 'scraper':"latest_trailers" },
					  {'Title':self.language( 30034 )  ,'path':"/root/explore/trailers/current"  		, 'thumbnail':"trailers"			, 'login':"false" , 'scraper':"current_trailers" },
					  {'Title':self.language( 30036 )  ,'path':"/root/explore/trailers/upcoming" 		, 'thumbnail':"trailers"			, 'login':"false" , 'scraper':"upcoming_trailers" },
					  {'Title':self.language( 30033 )  ,'path':"/root/explore/trailers/popular"  		, 'thumbnail':"trailers"			, 'login':"false" , 'scraper':"popular_trailers" },
					  {'Title':self.language( 30047 )  ,'path':"/root/explore/trailers/latest_game"  	, 'thumbnail':"trailers"			, 'login':"false" , 'scraper':"latest_game_trailers" },
	...
	def addToFavorites(self, params = {}):
		get = params.get
		if (get("videoid")):
			(message, status) = self.feeds.add_favorite(params)
			if status != 200:
	...
	def removeFromFavorites(self, params = {}):
		...
		if (get("editid")):
			(message, status ) = self.feeds.delete_favorite(params)
			if status != 200:
				self.showErrorMessage(self.language(30020), message, status)
				return False
	...
	def addContact(self, params = {}):
		...
		if (get("contact")):
			(result, status) = self.feeds.add_contact(params)
			if status != 200:
	...
	def removeContact(self, params = {}):
		...
		if (get("contact")):
			(result, status) = self.feeds.remove_contact(params)
			if status != 200:
				self.showErrorMessage(self.language(30029), result, status)
				return False
	...
	def addSubscription(self, params = {}):
		get = params.get
		if (get("channel")):
			(message, status) = self.feeds.add_subscription(params)
			if status != 200:
	...
	def removeSubscription(self, params = {}):
		get = params.get
		if (get("editid")):
			(message, status) = self.feeds.remove_subscription(params)
			if status != 200:
				self.showErrorMessage(self.language(30021), message, status)
				return False
	...
	def addFolderContextMenuItems(self, params = {}, item_params = {}):
		...
		if (item("user_feed") == "favorites" or item("user_feed") == "newsubscriptions"):
			cm.append ( (self.language(30520), "XBMC.RunPlugin(%s?path=%s&action=play_all&user_feed=%s&contact=%s&)" % ( sys.argv[0], item("path"), item("user_feed"), "default" ) ) )
			cm.append ( (self.language(30522), "XBMC.RunPlugin(%s?path=%s&action=play_all&shuffle=true&user_feed=%s&contact=%s&)" % ( sys.argv[0], item("path"), item("user_feed"), "default" ) ) )
		if (item("playlist")):
			cm.append ( (self.language(30531), "XBMC.RunPlugin(%s?path=%s&action=reverse_order&playlist=%s&)" % ( sys.argv[0], item("path"), item("playlist") ) ) )
			cm.append ( (self.language(30520), "XBMC.RunPlugin(%s?path=%s&action=play_all&playlist=%s&)" % ( sys.argv[0], item("path"), item("playlist") ) ) )
			cm.append ( (self.language(30522), "XBMC.RunPlugin(%s?path=%s&action=play_all&shuffle=true&playlist=%s&)" % ( sys.argv[0], item("path"), item("playlist") ) ) )
			if not get("external"):
